...
export const FormEditMahasiswa = () => {
  ...
  useEffect(() => {
    const getMahasiswaById = async () => {
      try {
        const response = await axios.get(`${getApiBaseUrl()}/mahasiswa/${id}`, { 
        ...
        if (response.data.avatar) {
          setPreview(`${getApiBaseUrl()}/uploads/mahasiswa/${response.data.avatar}`);
        }
      } catch (error) {
  ...
  const updateMahasiswa = async (e) => {
    ...
    try {
      ...
      await axios.put(`${getApiBaseUrl()}/mahasiswa/${id}`, formData, {
      ...
      if (error.response?.data?.msg === 'Username sudah digunakan') {
        ...
        setMessage(error.response?.data?.msg || 'Terjadi kesalahan saat memperbarui mahasiswa');
      }
    }
  };
  ...
  const cabangName = cabangs.find(cabang => cabang.id === cabangId)?.nama || 'Cabang tidak ditemukan';
  
  return (
    <DashboardContainer>
      <Paper elevation={0} sx={{ borderRadius: 3, overflow: 'hidden', bgcolor: '#f8fafc' }}>
        <Box sx={{ p: 2, bgcolor: '#1976d2', color: 'white' }}>
          <Typography variant="h6">Informasi Mahasiswa</Typography>
        </Box>
        <Box sx={{ p: 3 }}>
            ...
            <Grid item xs={12} md={8}>
              <form onSubmit={updateMahasiswa}>
                ...
                <Box sx={{ mt: 4, display: 'flex', gap: 2 }}>
                  ...
                  <Button 
                    variant="outlined" 
                    onClick={() => navigate('/datamahasiswa')}
                    size="large"
                    sx={{ borderRadius: 2 }}
                  >
                    Batal
            ...
            <Grid item xs={12} md={4}>
              <StyledCard>
                <CardHeader>
                  <AvatarWrapper>
                    <StyledAvatar
                      src={preview || (avatar ? `${getApiBaseUrl()}/uploads/mahasiswa/${avatar}` : '')}
                      alt={namaLengkap}
                    />
                  </AvatarWrapper>
                </CardHeader>
